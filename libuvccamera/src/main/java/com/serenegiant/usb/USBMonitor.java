...
public final class USBMonitor {
	...
	public USBMonitor(final Context context, final OnDeviceConnectListener listener) {
		if (DEBUG) Log.v(TAGG, "USBMonitor:Constructor");
		if (listener == null)
...
//		if (DEBUG) Log.v(TAG, "requestPermission:device=" + device);
	...
	public final void processConnect(final UsbDevice device) {
		...
		mAsyncHandler.post(new Runnable() {
			@Override
			public void run() {
				if (true) Log.v(TAGG, "processConnect:device=" + device);
                Log.v(TAGG,"来自USBMonitor中的processConnect方法被调用了");
                UsbControlBlock ctrlBlock;
...
//			if (DEBUG) {
				if (mConnection != null) {
					...
					Log.i(TAG, String.format(Locale.CHINA, "name=%s,desc=%d,busnum=%d,devnum=%d,rawDesc=", name, desc, busnum, devnum) + rawDesc);
				} else {
